      ...
      <Modal.Body>
        <Form>
          <Dropdown className="mt-2 mb-2">
            ...
            <DropdownMenu>
              {types.map((type) => (
                <DropdownItem onClick={() => dispatch(selectType(type))} key={type.id}>
                  {type.name}
                </DropdownItem>
              ))}
            </DropdownMenu>
          ...
          <Dropdown className="mt-2 mb-2">
            ...
            <DropdownMenu>
              {brands.map((brand) => (
                <DropdownItem onClick={() => dispatch(selectBrad(brand))} key={brand.id}>
                  {brand.name}
                </DropdownItem>
              ))}
            </DropdownMenu>
          </Dropdown>
          <FormControl value={name} onChange={(e) => setName(e.target.value)} className="mt-3" placeholder="Enter Device Name" />
          <FormControl
            value={price}
